...
class SubtitleAssembler:
    ...
    def reset(self) -> None:
        ...
        self._required_agreement_count = 2
    ...
    def _update_word(target: _WordState, incoming: _WordState, *, increment_count: bool) -> None:
        if increment_count:
            target.count += 1
        target.start = (target.start + incoming.start) / 2.0
        target.end = (target.end + incoming.end) / 2.0
        target.score = (target.score + incoming.score) / 2.0
