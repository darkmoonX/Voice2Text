...
QByteArray convertToPcm16Interleaved(const BYTE *data, UINT32 frames, const WAVEFORMATEX *format,
                                     bool isSilent) {
    const int channels = std::max<int>(1, static_cast<int>(format != nullptr ? format->nChannels : 1));
    QByteArray out;
    out.resize(static_cast<int>(frames * static_cast<UINT32>(channels) * sizeof(int16_t)));
    auto *dst = reinterpret_cast<int16_t *>(out.data());

    if (isSilent || data == nullptr || format == nullptr) {
        std::fill(dst, dst + (frames * channels), 0);
        return out;
    }

    if (format->wBitsPerSample == 16) {
        const auto *src = reinterpret_cast<const int16_t *>(data);
        std::copy(src, src + (frames * channels), dst);
        return out;
    }
    ...
    if (format->wBitsPerSample == 32) {
        ...
        for (int i = 0; i < totalSamples; ++i) {
            ...
            const int sample = std::clamp(static_cast<int>(std::lround(v * 32767.0F)), -32768, 32767);
...
bool queryRenderEndpointMixFormat(const QString &deviceId,
                                  WAVEFORMATEX **outFormat,
                                  QString *statusOut,
                                  QString *errorOut) {
    if (outFormat == nullptr) {
        if (errorOut != nullptr) {
            *errorOut = "Invalid outFormat pointer.";
        }
        return false;
    }
    *outFormat = nullptr;
    ...
    IAudioClient *endpointClient = nullptr;
    WAVEFORMATEX *endpointFormat = nullptr;
    bool ok = false;
    ...
    if (FAILED(hr) || enumerator == nullptr) {
        if (errorOut != nullptr) {
            *errorOut = "Unable to create MMDeviceEnumerator for endpoint format query.";
        }
        goto cleanup;
    }
    ...
    if (FAILED(hr) || device == nullptr) {
        if (errorOut != nullptr) {
            *errorOut = "Render endpoint unavailable for format query.";
        }
        goto cleanup;
    }

    hr = device->Activate(__uuidof(IAudioClient), CLSCTX_ALL, nullptr,
                          reinterpret_cast<void **>(&endpointClient));
    if (FAILED(hr) || endpointClient == nullptr) {
        if (errorOut != nullptr) {
            *errorOut = "Unable to activate endpoint IAudioClient for format query.";
        }
        goto cleanup;
    }

    hr = endpointClient->GetMixFormat(&endpointFormat);
    if (FAILED(hr) || endpointFormat == nullptr) {
        if (errorOut != nullptr) {
            *errorOut = QString("Endpoint GetMixFormat failed (hr=0x%1)")
                            .arg(static_cast<quint32>(hr), 8, 16, QLatin1Char('0'));
        }
        goto cleanup;
    }

    *outFormat = endpointFormat;
    endpointFormat = nullptr;
    ok = true;
    if (statusOut != nullptr) {
        *statusOut = QString("%1 Hz, %2 ch, %3-bit")
                         .arg(static_cast<int>((*outFormat)->nSamplesPerSec))
                         .arg(static_cast<int>(std::max<WORD>(1, (*outFormat)->nChannels)))
                         .arg(static_cast<int>((*outFormat)->wBitsPerSample));
    }

cleanup:
    if (endpointFormat != nullptr) {
        CoTaskMemFree(endpointFormat);
    }
    safeRelease(&endpointClient);
    safeRelease(&device);
...
void LoopbackCapture::captureLoop() {
    const DWORD coInitFlags = (mode_ == CaptureSourceMode::App)
    ...
    WAVEFORMATEX processLoopbackFallbackFormat{};
    bool ownsMixFormat = false;
    ...
    REFERENCE_TIME processLoopbackDuration = bufferDuration;
    bool useDefaultEndpoint = true;
    bool processLoopbackPath = false;
    ...
    processLoopbackPath = (mode_ == CaptureSourceMode::App && !useDefaultEndpoint);
    if (processLoopbackPath) {
        QString endpointFormatStatus;
        QString endpointFormatError;
        if (queryRenderEndpointMixFormat(requestedDeviceId, &mixFormat, &endpointFormatStatus, &endpointFormatError)) {
            ownsMixFormat = true;
            emit statusRaised(QString("Process loopback using endpoint mix format: %1").arg(endpointFormatStatus));
        } else {
            // Process-loopback virtual client may not implement GetMixFormat/IsFormatSupported.
            // Fall back to a stable PCM format only when endpoint-format query is unavailable.
            processLoopbackFallbackFormat.wFormatTag = WAVE_FORMAT_PCM;
            processLoopbackFallbackFormat.nChannels = 2;
            processLoopbackFallbackFormat.nSamplesPerSec = 44100;
            processLoopbackFallbackFormat.wBitsPerSample = 16;
            processLoopbackFallbackFormat.nBlockAlign = static_cast<WORD>(
                processLoopbackFallbackFormat.nChannels * processLoopbackFallbackFormat.wBitsPerSample / 8);
            processLoopbackFallbackFormat.nAvgBytesPerSec =
                processLoopbackFallbackFormat.nSamplesPerSec * processLoopbackFallbackFormat.nBlockAlign;
            processLoopbackFallbackFormat.cbSize = 0;
            mixFormat = &processLoopbackFallbackFormat;
            ownsMixFormat = false;
            emit statusRaised(
                QString("Process loopback endpoint mix-format query failed (%1); using fallback PCM 44100/2ch/16-bit.")
                    .arg(endpointFormatError.isEmpty() ? QString("unknown") : endpointFormatError));
        }
    } else {
    ...
    processLoopbackDuration = (mode_ == CaptureSourceMode::App && !useDefaultEndpoint) ? 0 : bufferDuration;
    hr = audioClient->Initialize(
        AUDCLNT_SHAREMODE_SHARED,
        streamFlags,
        processLoopbackDuration,
        0,
        mixFormat,
        nullptr);
    if (FAILED(hr)) {
    ...
    while (running_.load()) {
        ...
        while (packetLength > 0 && running_.load()) {
            ...
            const QByteArray pcm = convertToPcm16Interleaved(data, frames, mixFormat, isSilent);
            emit pcmChunkReady(
                pcm,
                static_cast<int>(mixFormat->nSamplesPerSec),
                static_cast<int>(std::max<WORD>(1, mixFormat->nChannels)));
...
cleanup:
    ...
    if (mixFormat != nullptr && ownsMixFormat) {
