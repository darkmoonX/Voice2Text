...
def _build_whispercpp_diarizer(config: RuntimeConfig, *, device: str, progress_callback: Callable[[str], None] | None):
    """Construct a WhisperCppDiarizer if diarization is enabled, else None.

    Shared by both whisper.cpp execution modes (round 0067) so the subprocess-mode
    transcriber and the server-mode transcriber's automatic subprocess fallback get the
    identical diarizer instance/config as the primary server-mode path (round 0065/0066).
    """
    if not bool(getattr(config, 'whisperx_enable_diarization', False)):
        return None
...
def _build_whispercpp(config: RuntimeConfig, *, device_override: Optional[str], progress_callback: Callable[[str], None] | None) -> STTTranscriber:
    ...
    diarizer = _build_whispercpp_diarizer(config, device=device, progress_callback=progress_callback)
...
def _build_whisperx(config: RuntimeConfig, *, device_override: Optional[str], compute_type_override: Optional[str], progress_callback: Callable[[str], None] | None) -> STTTranscriber:
    ...
    return WhisperXTranscriber(
        ...
        asr_temperatures=str(getattr(config, 'whisperx_asr_temperatures', '') or ''),
        asr_log_prob_threshold=getattr(config, 'whisperx_asr_log_prob_threshold', None),
        asr_compression_ratio_threshold=getattr(config, 'whisperx_asr_compression_ratio_threshold', None),
        asr_no_speech_threshold=getattr(config, 'whisperx_asr_no_speech_threshold', None),
