...
def build_arg_parser(whisper_defaults: WhisperRuntimeParams) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", default="auto", help="Model name used by the selected STT provider. 'auto' resolves by effective device: large-v3 on CUDA, small on CPU (round 0072).")
    ...
    parser.add_argument("--speaker-count-hint", dest="speaker_count_hint_enabled", action="store_true", help="Round 0055: periodically estimate speaker count from a bounded rolling direct diarization window and raise the live profile cap. Default off.")
    parser.add_argument("--no-speaker-count-hint", dest="speaker_count_hint_enabled", action="store_false", help="Round 0055: disable automatic speaker-count cap feedback.")
    parser.add_argument("--speaker-count-hint-seconds", type=float, default=60.0, help="Round 0055: audio-seconds cadence between automatic speaker-count estimation passes.")
    parser.add_argument("--speaker-count-hint-window-seconds", type=float, default=300.0, help="Round 0055: rolling recent-audio window analyzed by each speaker-count pass.")
    parser.add_argument("--speaker-count-hint-sliver-floor-seconds", type=float, default=1.5, help="Round 0055: drop diarization clusters shorter than this before counting speakers.")
    parser.add_argument("--speaker-merge-grace-windows", type=int, default=None, help="Post-merge online-match grace window count. 0/unset disables.")
    parser.add_argument("--speaker-merge-grace-relief", type=float, default=None, help="Temporary cosine-threshold relief while a merged profile is in its grace window.")
    parser.add_argument("--speaker-merge-preserve-centroid", dest="speaker_merge_preserve_centroid", action="store_true", default=None, help="Preserve the survivor centroid during profile merge instead of weighted-average blending.")
    parser.add_argument("--no-speaker-merge-preserve-centroid", dest="speaker_merge_preserve_centroid", action="store_false", help="Use weighted-average centroid blending during profile merge.")
    parser.add_argument("--speaker-profile-max-exemplars", type=int, default=None, help="Round 0061: bounded multi-exemplar profile representation. 1/unset = single-centroid (byte-identical). >1 lets a profile keep several distinct representative embeddings; gated to zh/CJK sessions only as of round 0062, behaves as 1 on other languages.")
    parser.add_argument("--speaker-profile-exemplar-diversity-threshold", type=float, default=None, help="Round 0061: similarity threshold deciding blend-into-nearest-exemplar vs. add-a-new-exemplar. Only consulted when max-exemplars > 1.")
    ...
    parser.add_argument("--diarization-min-speakers", type=int, default=None, help="Optional expected minimum speaker count for WhisperX diarization. 0/unset = auto.")
    parser.add_argument("--diarization-max-speakers", type=int, default=None, help="Optional expected maximum speaker count for WhisperX diarization and live profile cap. 0/unset = auto.")
    ...
    parser.add_argument("--subtitle-relabel-async", dest="subtitle_relabel_async", action="store_true", help="Round 0052 Phase B: resolve relabel spans on a background worker thread instead of the loop thread (avoids the synchronous diar stall at sentence breaks).")
    parser.add_argument("--no-subtitle-relabel-async", dest="subtitle_relabel_async", action="store_false", help="Round 0052 Phase B: force the synchronous relabel path.")
    parser.set_defaults(speaker_count_hint_enabled=False)
    parser.set_defaults(subtitle_relabel_async=False)
    parser.add_argument("--asr-temperatures", type=str, default="0.0,0.2,0.4", help="Round 0049: comma-separated temperature-fallback schedule override for WhisperX ASR. Default trims worst-case re-decodes on hard windows with byte-identical quality; pass '' to restore the library's full 6-step schedule.")
