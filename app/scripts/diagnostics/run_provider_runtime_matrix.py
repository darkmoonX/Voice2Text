"""WhisperX runtime boot check and timed transcription probe."""
...
def run_provider_for_duration(
    ...
    progress(
        ...
        + "; whisperx_internal_vad="
        + ("on" if bool(getattr(cfg, "whisperx_enable_vad", True)) else "off")
    )
    init_ok = False
    init_err = ""
    try:
    ...
    if not init_ok and init_err:
        ...
        logger.info(
            "[%s] completed: iterations=%s success=%s errors=%s",
            provider,
            iterations,
            success_count,
            len(errors),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run WhisperX runtime probe.")
    parser.add_argument(
        "--providers",
        default="whisperx",
        help="Compatibility option. Only whisperx is supported.",
    )
    parser.add_argument(
        "--duration-seconds",
        type=int,
    ...
    for item in results:
        print(
            ...
            f"success={item.success_count} errors={item.error_count}",
            flush=True,
        )
